...
func home(c echo.Context) error {
	...
	data, _ := connection.Conn.Query(context.Background(), "SELECT id, title, content, image, startdate, enddate FROM tb_blog")

	var result []Blog
	for data.Next() {
		var each = Blog{}
		err := data.Scan(&each.Id, &each.Title, &each.Content, &each.Image, &each.StartDate, &each.EndDate)
		...
		date1, _ := time.Parse("2006-01-02", each.StartInput)
		date2, _ := time.Parse("2006-01-02", each.EndInput)
		...
		each.StartDate = NewStartDate
		each.EndDate = NewEndDate

		result = append(result, each)
	}

	dataBlog := map[string]interface{}{
		"blogHome": append(dataBlog, result...),
	}
