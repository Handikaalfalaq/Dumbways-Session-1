...
func blogDetail(c echo.Context) error {
	...
	var blogData Blog
	err = connection.Conn.QueryRow(context.Background(), "SELECT tb_blog.title, content, image, startdate, enddate, technology, tb_user.name AS author FROM tb_blog INNER JOIN tb_user ON tb_blog.author_id = tb_user.id WHERE tb_blog.id = $1", id).Scan(&blogData.Title, &blogData.Content, &blogData.Image, &blogData.StartDate, &blogData.EndDate, &blogData.Technology, &blogData.Author)
...
func formRegister(c echo.Context) error {
	...
	notif, _ := session.Get("notif", c)
	flash := map[string]interface{}{
		"FlashStatus":  notif.Values["status"],
		"FlashMessage": notif.Values["message"],
	}
	delete(notif.Values, "status")
	delete(notif.Values, "message")
